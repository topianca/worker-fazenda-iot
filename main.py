...
cursor = conn.cursor()
# Usando a tabela 'leituras_cafe' que apareceu no seu print!
...
Configuração do Cliente MQTT com TLS (obrigatório para HiveMQ Cloud)
client = mqtt.Client(client_id="Railway_Worker_Thiago", transport="tcp")
client.username_pw_set(MQTT_USER, MQTT_PASS)
client.tls_set() # Habilita SSL/TLS
...
client.loop_forever() # Mantém o script rodando infinitamente
except Exception as e:
print(f"🔴 [ERRO CRÍTICO] Falha ao iniciar worker: {e}")
